...
| `list_open_nodes(session_id, root_id?, state?, type?, limit?, cursor?, fields?, text_preview?)` | **[v0.11]** Open nodes in session or within one root. Default: summary projection `{id,type,text,parent_id,parent_kind,state,severity}`, up to 50 per page; returns `next_cursor` (follow until null to read all). `fields="*"` returns full 19-column rows. `limit` 1–200. `text_preview=N` adds `text_truncated`+`text_len`. `type`/`state` filter (AND'd). `cursor` is opaque and valid only for the same filter args. |
| `list_unblocked_open_nodes(session_id, root_id?, blocker_edge_type?, state?, type?, limit?, cursor?, fields?, text_preview?)` | **[v0.11]** Open nodes that no open node is blocking via the given edge type (default `'blocks'`). Same pagination/projection contract as `list_open_nodes`: summary default, `next_cursor`, `fields="*"` for full rows, `limit` 1–200, `state` filter (new, parity with `list_open_nodes`). |
