...
このコマンド 1 つで、リポジトリのクローン (デフォルトでは `~/agent-dpd`)、仮想環境の作成、パッケージのインストール、`dpd-mcp-server` の Claude Code への登録までを行います。実行後は Claude Code を再起動してください — `/dpd` スキルが利用可能になります。
...
| `DPD_NO_REGISTER` | 未設定 | 設定すると Claude Code への登録をスキップ |
