...
| [`dpd-dump`](dpd-dump/) | DPD グラフ全体をテキストに dump (default は tree、`--format=mermaid` で Mermaid)。audit / snapshot / docs への貼り付け用。 |
...
`dpd-import → dpd-fill → /fcot` パイプラインは v0.3.1 spec 自体を検査した **self-validation** フロー。dogfood の経緯は top-level [`README.ja.md`](../README.ja.md#dpd-で-agent-driven-に作られた) 参照。

## インストール

skill family は Claude Code が discover するために `~/.claude/skills/dpd/` (および sub-skill サブディレクトリ) に展開される。このディレクトリから手動で sync する場合:
...
リポジトリのルートから `make install` で MCP server を入れた場合、Makefile に `skill-install` target を追加することもできる (TODO、未提供 — 今は上記の手動 `cp` が documented path)。
